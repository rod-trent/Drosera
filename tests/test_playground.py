...
# --- the precomputed scenarios ------------------------------------------


def test_committed_scenarios_match_a_fresh_computation():
    """web/scenarios.json is served instead of calling the engine, so it has to
    still be what the engine would say.

    Without this, a weight change would leave the site confidently serving last
    release's numbers -- the exact drift the demo was built to avoid by running
    the real engine in the first place.
    """
    import json

    committed = WEB_API.parent / "scenarios.json"
    assert committed.is_file(), "run: python web/make_scenarios.py"
    saved = json.loads(committed.read_text(encoding="utf-8"))

    assert saved["drosera"] == assess.__version__, (
        "scenarios.json was generated by a different version -- regenerate it"
    )
    for key in assess.SCENARIOS:
        assert saved["scenarios"][key] == assess.run_scenario(key), (
            f"scenarios.json is stale for {key!r}; run: python web/make_scenarios.py"
        )


def test_precomputed_scenarios_carry_the_headline_claim():
    """Whatever the site serves statically must still make the point."""
    import json

    saved = json.loads((WEB_API.parent / "scenarios.json").read_text(encoding="utf-8"))
    s = saved["scenarios"]
    assert s["crawler"]["agency"] == 0.0
